...
class DesignRatingApp {
    ...
    addTagToChat(cardId, text) {
        ...
        tagElement.className = 'chat-tag';
        tagElement.innerHTML = `
            <span class="chat-tag-text">${text}</span>
            <button class="chat-tag-remove" onclick="this.parentElement.remove(); app.updateChatStateAfterTagChange()">×</button>
        `;
        
        mainChatTags.appendChild(tagElement);
        
        // Update chat state based on current state
        this.updateChatStateAfterTagChange();
    }
    ...
    addButtonEventListeners() {
        ...
        document.querySelectorAll('.dust-card__add-to-chat').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                this.addCardToChat(cardType, cardContent);
            });
        });
        ...
        document.querySelectorAll('.dust-argument__add-to-chat').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                this.addArgumentToChat(title, content, parentCard);
            });
        });
    }
    ...
    addArgumentToChat(title, content, parentCard) {
        const mainChatTags = document.getElementById('mainChatTags');
        const floatingChat = document.getElementById('floatingChat');
        
        // Create a tag element for the argument
        const tagElement = document.createElement('div');
        tagElement.className = 'chat-tag';
        tagElement.innerHTML = `
            <span class="chat-tag-text">${this.getArgumentEmoji(this.detectArgumentType(content))} ${title}</span>
            <button class="chat-tag-remove" onclick="this.parentElement.remove(); app.updateChatStateAfterTagChange()">×</button>
        `;
        
        // Store the full content in the tag
        ...
        console.log(`Added "${title}" argument to chat tags`);
    }
    ...
    addCardToChat(title, content) {
        const mainChatTags = document.getElementById('mainChatTags');
        const floatingChat = document.getElementById('floatingChat');
        
        // Create a tag element for the card with the full content
        const tagElement = document.createElement('div');
        tagElement.className = 'chat-tag';
        tagElement.innerHTML = `
            <span class="chat-tag-text">📋 ${title}</span>
            <button class="chat-tag-remove" onclick="this.parentElement.remove(); app.updateChatStateAfterTagChange()">×</button>
        `;
        
        // Store the full content in the tag for later use when sending messages
        ...
        console.log(`Added "${title}" card content to chat tags`);
    }
